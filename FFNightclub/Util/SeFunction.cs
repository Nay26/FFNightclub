...
namespace FFNightClub.Util
{
    ...
    public class SeFunction<T> where T : Delegate
    {
        ...
        {
            Address = FFNightClub.SigScanner.Module.BaseAddress + offset;
        }

        public SeFunction(string signature, int offset = 0)
        {
            Address = FFNightClub.SigScanner.ScanText(signature);
            if (Address != IntPtr.Zero)
                Address += offset;
            var baseOffset = (ulong)Address.ToInt64() - (ulong)FFNightClub.SigScanner.Module.BaseAddress.ToInt64();
        }
        ...
        public Hook<T>? CreateHook(T detour)
        {
            if (Address != IntPtr.Zero)
            {
                var hook = FFNightClub.GameInteropProvider.HookFromAddress<T>(Address, detour);
                hook.Enable();
                return hook;
            }
